            ...
            <div className="flex flex-col w-70 md:w-[30%]  bg-gray-200 text-black rounded h-60 overflow-x-hidden">
                ...
                <div  className="flex flex-col  items-center px-1 ">
                    {loggedUsers.length >0 && loggedUsers?.map((user,index)=>(
                        <div className="flex flex-row w-full items-center rounded h-9 md:h-12 border-1 my-1 hover:bg-white cursor-pointer" key={index}>
                            <span className="bg-green-600 rounded-full w-4 h-4 p-2 mx-1"/>
                            <p className="w-2/3">{ccapitalizer_3000(user.username)}</p>
                            <p className="w-2/3 text-end pr-2">{ccapitalizer_3000(user.localidad)}</p>
                        </div>
                        ))}
                </div>
